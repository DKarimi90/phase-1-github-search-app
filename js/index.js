document.addEventListener("DOMContentLoaded", () => {
    // your code here
    let form = document.querySelector('form')
    form.addEventListener('submit', (e) => {
      e.preventDefault()
      searchInput(e.target.search.value)

    })

})

const config ={
    header:{
        "Content-Type": "application/json",
        Accept: "application/vnd.github.v3+json"
    }
}

function searchInput(input){
    fetch(`https://api.github.com/search/users?q=${input}`)
    .then(response => response.json())
    .then(data =>{

        let searchedName = data.items

        const lp = ()=>{
            for(i = 0; i <searchedName.length;i++){
                const user = document.querySelector('#user-list')
                const list = document.createElement('li')
                list.innerHTML=`<li>${searchedName[i].login}</li><p>${searchedName[i].html_url}<p><img src=${searchedName[i].avatar_url}>`
                user.appendChild(list)

                list.addEventListener("click",()=>{
                    fetch(`https://api.github.com/users/${searchedName[i].login}/repos`)
                    .then(response=>response.json())
                    .then(repos=>{
                        console.log(repos)
                        for(let i = 0; i <repos.length;i++){
                            const reposi =document.querySelector("#repos-list")
                            const repoli =document.createElement('li')
                            repoli.innerHTML = `${repos[i].name}`
                            reposi.appendChild(repoli)

                        }

                    })
                })

            }


        }

        lp()

    })

}